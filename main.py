...
from services.auth.login_wall import rendered_login_wall
...
def main():
    ...
    if rendered_login_wall():
        ...
        with st.sidebar:
            st.title('🏋️‍♀️ Advanced AI Coach')
            ...
            if not workout_started:
                ...
                start_session_button = st.button('▶️ Start Session', width="stretch", key="start_session_button")
        ...
        if isinstance(user_id, int):
            ...
            if not df.empty:
                ...
                df_agg = df.groupby(['Exercise','Date']).agg({
                    "Reps":"sum",
                    "Sets":"sum",
                    "Time (sec)":"sum"
                }).reset_index()
                ...
                st.table(df_agg, border="horizontal")
            else:
                st.info("No Workout History")
